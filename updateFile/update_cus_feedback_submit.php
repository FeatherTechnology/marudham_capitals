...
if($feedbackID == ''){
    $insert_qry = $connect->prepare("INSERT INTO `verification_cus_feedback`( `cus_id`, `feedback_label`,  `cus_feedback_dept`, `cus_feedback`,`upload`,`feedback_remark`,`insert_login_id`,`inserted_date`) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
    
    $insert_qry->execute([$cus_id, $feedback_label, $cus_feedback_dept, $cus_feedback, $uploadedFiles, $feedback_remark, $user_id]);
    ...
    $update = $connect->prepare("UPDATE `verification_cus_feedback` SET `cus_id` = ?, `feedback_label` = ?, `cus_feedback_dept` = ?, `cus_feedback` = ?, `upload` = ?, `feedback_remark` = ? WHERE `id` = ?");

    $update->execute([$cus_id, $feedback_label, $cus_feedback_dept, $cus_feedback, $uploadedFiles, $feedback_remark, $feedbackID]);
}
