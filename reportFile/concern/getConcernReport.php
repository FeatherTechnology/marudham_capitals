...
$query = "SELECT cc.id, cc.com_code,cc.com_date,cc.raising_for,cc.self_name,cc.cus_name,cs.concern_subject,cc.com_remark,sc.staff_name, cc.status,cc.solution_date,cc.communication,cc.location, cc.sol_participants,cc.solution_remark,cc.uploads ,ag.ag_name,scs.staff_name AS concern_against_staff FROM concern_creation cc LEFT JOIN concern_subject cs ON cc.com_sub = cs.concern_sub_id LEFT JOIN agent_creation ag ON cc.ag_name = ag.ag_id LEFT JOIN staff_creation sc ON cc.staff_assign_to = sc.staff_id LEFT JOIN staff_creation scs
ON cc.concern_against = scs.staff_id
WHERE $where ";
...
foreach ($result as $row) {
    ...
    $sub_array[] = isset($row['concern_against_staff']) ? $row['concern_against_staff'] : '';
