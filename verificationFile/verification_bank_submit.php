...
if ($bankID == '') {
    ...
    $insert_qry = $connect->query("INSERT INTO `verification_bank_info`(`cus_id`, `req_id`, `bank_name`, `branch_name`, `acc_holder_name`, `acc_no`, `ifsc_code`,`upload`) VALUES ('$cus_id','$req_id','$bank_name','$branch_name','$account_holder_name','$account_number','$Ifsc_code','$uniqueFileName')");
} else {
    ...
    $insert_qry = $connect->query("UPDATE `verification_bank_info` SET `cus_id`='$cus_id',`req_id`='$req_id',`bank_name`='$bank_name',`branch_name`='$branch_name',`acc_holder_name`='$account_holder_name',`acc_no`='$account_number',`ifsc_code`='$Ifsc_code',`upload`='$bank_upload_id' WHERE `id`='$bankID'");
}
