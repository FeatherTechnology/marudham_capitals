...
try {
    ...
    $stmt = $connect->prepare(
        "UPDATE request_creation 
         SET cus_status = 3, updated_date = NOW(), update_login_id = ? 
         WHERE req_id = ?"
    );
    ...
    $stmt = $connect->prepare(
        "UPDATE customer_register 
         SET cus_status = 3, updated_date = NOW() 
         WHERE req_ref_id = ?"
    );
    $stmt->execute([$req_id]);
