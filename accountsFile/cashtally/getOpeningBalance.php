...
function getOpeningBalance($connect, $op_date, $bank_detail, $user_id)
{
    ...
    $agentCollQry = $connect->query("SELECT
        ...
        FROM (
            ...
            FROM collection cl
            WHERE created_date BETWEEN '$op_date_start' AND '$op_date_time'
            $ag_where  
    ...
    $agentIssueQry = $connect->query("SELECT 
        ...
        FROM (
            ...
            FROM loan_issue 
            WHERE 
                created_date BETWEEN '$op_date_start' AND '$op_date_time'
    ...
    $agentCreditQry = $connect->query("SELECT
        ...
        FROM (
            (SELECT COALESCE(SUM(amt), 0) AS amt FROM ct_cr_hag WHERE created_date BETWEEN '$op_date_start' AND '$op_date_time' AND FIND_IN_SET(ag_id, '$ag_ids') $user_where)
    ...
    $agentDebitQry = $connect->query("SELECT
        ...
        FROM (
            (SELECT COALESCE(SUM(amt), 0) AS amt FROM ct_db_hag WHERE created_date BETWEEN '$op_date_start' AND '$op_date_time' AND FIND_IN_SET(ag_id,'$ag_ids') $user_where)
    ...
    $agentCreditQry = $connect->query("SELECT
        SUM(amt) AS agent_credit
        FROM (
            (SELECT COALESCE(SUM(amt), 0) AS amt FROM ct_cr_bag WHERE created_date BETWEEN '$op_date_start' AND '$op_date_time' AND FIND_IN_SET(ag_id,'$ag_ids') $user_where)
    ...
    $agentDebitQry = $connect->query("SELECT
        SUM(amt) AS agent_debit
        FROM (
            (SELECT COALESCE(SUM(amt), 0) AS amt FROM ct_db_bag WHERE created_date BETWEEN '$op_date_start' AND '$op_date_time' AND FIND_IN_SET(ag_id, '$ag_ids') $user_where)
...
FROM (
    ...
    FROM collection cl
    WHERE cl.created_date < DATE_FORMAT('$op_date', '%Y-%m-01') $ag_where
                    
    UNION ALL
    -- Loan issue
    SELECT 0 AS Credit, (IFNULL(li.cash,0) + IFNULL(li.cheque_value,0) + IFNULL(li.transaction_value,0)) AS Debit, 'coll_loan' AS grp
    FROM loan_issue li
    WHERE  li.created_date < DATE_FORMAT('$op_date', '%Y-%m-01') and FIND_IN_SET(li.agent_id,'$ag_ids') AND agent_id IS NOT NULL

    UNION ALL
    -- ct_db_hag → Debit
    SELECT 0 AS Credit, amt AS Debit, 'ct_tables' AS grp
    FROM ct_db_hag
     WHERE created_date < DATE_FORMAT('$op_date', '%Y-%m-01')
                        $user_where
